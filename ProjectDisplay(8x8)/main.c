...
void ShowFrame(int curr_frame, PatternS pat){
	...
	if ( !(SPI2->SR & SPI_SR_BSY) && isDataPushedToSPI){
		GPIOA->BSRR = GPIO_BSRR_BR_8;
		switch(curr_frame){
			case 0:{
				if (!pat.showframe0) return;
				SPI2->DR = pat.frame0;
				break;
				
			}
			case 1:{
				if (!pat.showframe1) return;
				SPI2->DR = pat.frame1;
				break;
			}
			case 2:{
				if (!pat.showframe2) return;
				SPI2->DR = pat.frame2;
				break;
			}
		}
...
void SysTick_Handler(void){ 
	ButtonsCheker();
	//SPI2->DR = globalDataForDisplay;
	ShowFrame(frame, patternStruct);
	ChangeDisplayFrame();
}
		...
		if (isDataPushedToSPI){
			...
			if	(posY >= 7) patternStruct.showframe2 = 0;
				...
				case 0:{
					...
					unsigned char column  = 0x01 << posY-1;
					data = ((unsigned short) pattern << 8) | column;
					break;
				}
				case 1:{
					unsigned char pattern = 0;
					if (posX == 0)
						...
						pattern = 0x07 << posX-1; // ..xxx..
					unsigned char column  = 0x01 << posY;
					data = ((unsigned short) pattern << 8) | column;
					break;
				}
				case 2:{
